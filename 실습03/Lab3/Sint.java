...
public class Sint {
		...
		if (ds != null) {
			for (Decl decl : ds)
	            if (decl.expr == null) {
	            	...
	            	// 할당값이 없는 경우(<type> id;)에 대한 allocate 함수 구현부이다. decl.id와 id.type을 value로 설정한 defaultValue로 push해준다.
	            	state.push(decl.id, new Value(decl.id.type));
	            }
		        else {
		        	// TODO: [push a new variable with id and value of V(expr)]
		        	//할당값이 있는 경우이다. decl.expr을 상태변환한 값을 value로 하여 push해준다.
		            state = state.push(decl.id, V(decl.expr, state));
		        }	
			return state;
		}
    ...
    State free (Decls ds, State state) {
    	...
    	if (ds != null) {
    		for (Decl decl : ds) {
    			...
    			// Decls ds에 대해 반복하며 decl이 있을 경우 state에서 pop을 해준다. 그 후, state를 리턴한다.
    			state.pop();
    		}
    		return state;
    	}
    ...
    Value binaryOperation(Operator op, Value v1, Value v2) { // Lab03
        ...
        case "/": 
            ...
            // <, <=, >, >= 연산들은 문자값과 정수값 모두 연산이 가능한 연산자 이기 때문에 타입이 문자일 때, 정수 일때로 경우를 나누어 각 타입에 대한 연산을 한 후 리턴한다.
            // ==, != 연산들은 이것들을 비롯해 bool연산까지 가능하기 때문에 bool에 대한 경우의 수를 하나 더 추가하여 각 타입에 대한 연산 후 리턴한다.
            // &&, || 연산은 bool에 대해서만 가능하기 때문에 bool 타입의 연산을 거쳐 리턴해준다.
        case "<":
