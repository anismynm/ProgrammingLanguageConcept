...
public class Sint {
    ...
    // [Function] : 함수 호출에 따른 새로운 스택 프레임을 생성하고, 함수의 매개변수를 전달하는 역할을 한다. 
    ...
    // [Function] : 함수 호출에 따라 생성된 스택 프레임을 해제하는 함수이다. f.params가 null이 아닐 경우 free함수를 통해 함수의 매개변수에 할당된 메모리를 해제한다.
    ...
    State Eval(Let l, State state) {
        ...
        // [Function] : let문을 평가하는 Eval함수이다. Let 문 내의 문장들을 평가하고, 평가된 상태를 s에 저장하고 Let 문에 할당된 메모리를 해제하고 최종 상태를 반환한다.
        if (l.funs != null) {
